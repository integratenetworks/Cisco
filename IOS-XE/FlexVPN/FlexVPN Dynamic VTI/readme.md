...
crypto ikev2 keyring KEYRING
 peer ANY-PEER
  ...
  exit
```
Create IKEv2 Profile (specify local identity of FQDN, match any peer on the domain name, specify authentication PSK, specify the Keyring to use)
```ruby
crypto ikev2 profile IKEV2_PROFILE
 match identity remote fqdn domain lab.net
...
crypto ipsec profile IPSEC_PROFILE
 set ikev2-profile IKEV2_PROFILE
```
Create a SVTI (use Lo0 as tunnel interface, specify tunnel source, tunnel destination as Hub’s WAN IP, specify IPSec Profile) 
```ruby
interface tunnel0
 ip unnumbered loopback 0
 tunnel source gigabitethernet 0/0
